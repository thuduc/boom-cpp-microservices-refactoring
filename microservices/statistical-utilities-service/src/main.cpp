...
private:
    ...
    void process_request() {
        auto response = handle_stats_request(std::move(req_));
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: statistical_utilities_service <address> <port>\n";
        std::cerr << "Example: statistical_utilities_service 0.0.0.0 8089\n";
        return 1;
    }
    ...
    std::cout << "Statistical Utilities Service listening on " << address << ":" << port << std::endl;
