...
#include <string>
#include <thread>
#include "api/special_functions_handler.hpp"
...
private:
    ...
    void process_request() {
        auto response = handle_special_functions_request(std::move(req_));
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: special_functions_service <address> <port>\n";
        std::cerr << "Example: special_functions_service 0.0.0.0 8088\n";
        return 1;
    }
    ...
    std::cout << "Special Functions Service listening on " << address << ":" << port << std::endl;
