...
private:
    ...
    void process_request() {
        auto response = handle_mixture_request(std::move(req_));
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: mixture_models_service <address> <port>\n";
        std::cerr << "Example: mixture_models_service 0.0.0.0 8087\n";
        return 1;
    }
    ...
    std::cout << "Mixture Models Service listening on " << address << ":" << port << std::endl;
